        ...
        Ok(mut file) => {
            ...
            match file.write_all(request.body.as_bytes()) {
                ...
                Err(e) => {
                    ...
                    return HttpResponse{code: HttpStatusCode::ServerError, body: body.into(), headers: vec![Header{name: "Content-Length".into(), value: format!("{}", content_length).into()}]}
                },
            }
...
fn handler(request: HttpRequest, config: &Config) -> HttpResponse {
    if request.path == "/" {
        return HttpResponse{code: HttpStatusCode::Success, body: Bytes::new(), headers: vec![]}
    }

    if request.path.starts_with("/echo/") {
        return echo(request, config)
    }
